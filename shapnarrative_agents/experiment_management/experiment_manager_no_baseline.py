...
class ExperimentManager():
    ...
    def run_experiments(
        ...
        for index, (dataset, tar_model_name, gen_model, prompt_type) in enumerate(itertools.product(self.dataset_names,self.tar_model_names,self.generation_models, self.prompt_types)):
            ...
            narratives=story_generator.generate_stories(tar_model,test_set[test_set.columns[0:-1]],test_set[test_set.columns[-1]], prompt_type=prompt_type, num_feat=self.num_feat, manipulate=manipulate)
            end_time=time.time()
