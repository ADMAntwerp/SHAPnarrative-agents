...
class ExperimentManager():
    ...
    """ 
    This document is specifically for the special use of running experiments with baseline narratives only. 
    """
    ...
    def __init__(
        ...
        # Version 2: store as list of (gen, ext) pairs
        if len(generation_models) != len(extraction_models):
            raise ValueError("For 1–1 pairing, generation_models and extraction_models must be the same length.")
        self.model_pairs = list(zip(generation_models, extraction_models))
    ...
    def run_experiments(
        ...
        # Version 2 case, the generation_models should have replaced with model_pairs, but normally they are the same number, so we keep the original 
        total_iterations = len(self.dataset_names) * len(self.tar_model_names) * len(self.generation_models)* len(self.prompt_types)  
        
        #Version 2: This version is for when we make generation and extraction model 1 to 1 pairs
        for index, (dataset, tar_model_name, (gen_model, extr_model), prompt_type) in enumerate(
        ...
        # for index, (dataset, tar_model_name, gen_model, prompt_type) in enumerate(
        # itertools.product(self.dataset_names,self.tar_model_names,self.generation_models, self.prompt_types)):
            ...
            #special setting: use the baseline narratives
            narratives = [
                self.get_baseline_narrative(dataset, idx)
                for idx in test_set.index
            ]

            # extract features and labels
            x = test_set[test_set.columns[:-1]]
            y = test_set[test_set.columns[-1]]

            story_generator.gen_variables(tar_model, x, y, tree=True)
            explanation_list=story_generator.explanation_list
            result_df=story_generator.result_df


            #Version 2: 
            # NO inner loop – use only the paired extractor
            ...
            #Version 1: This verion: generate extractions with every extraction model
            # extractions_dict={}
